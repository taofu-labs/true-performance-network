    ...
    for outcome in outcomes:
        ...
        conn.execute(
            """INSERT INTO scoring_results
               (run_id, hotkey, passed_floors, disqualified, disqualification_reason,
                actual_scores, final_score, max_memory_kb, lying_detected, eval_backend)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                run_id, r.hotkey, int(r.passed_floors), int(r.disqualified),
...
def is_scored(conn: sqlite3.Connection, competition_id: str) -> bool:
    row = conn.execute(
        "SELECT 1 FROM scored_competitions WHERE competition_id = ?", (competition_id,)
    ).fetchone()

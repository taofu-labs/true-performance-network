...
def _clear_poll_staleness_clocks():
    """poll_open_benchmarks keeps its per-(competition,hotkey,benchmark)
    staleness clock in module-level dicts (deliberately not persisted — see
    scorer.py). Tests reuse the same comp1/hk1/mmlu keys across cases, so
    without a reset a benchmark left mid-poll by one test (e.g. the
    pending-resume test) would leak its clock into the next test that
    reuses the same key."""
    scorer._last_log_at.clear()
    scorer._last_progress_wall.clear()
    yield
    scorer._last_log_at.clear()
    scorer._last_progress_wall.clear()
...
def test_poll_open_benchmarks_marks_pending_resume_when_window_still_open(monkeypatch):
    ...
    coordinator = NeverCompletingCoordinator()
    ...
    candidate = store.get_candidate(conn, "comp1", "hk1")
    assert candidate["status"] == "benchmarking"  # not resolved yet, still in-flight per its own semantics


def test_poll_open_benchmarks_hard_fails_when_window_closed(monkeypatch):
    monkeypatch.setattr(common_settings, "BENCHMARK_EXECUTION_STALE_SECONDS", 0)
    conn = make_db()
    submission = make_submission()
    insert_candidate(conn, "hk1", submission, status="benchmarking")
    ...
    spec = make_spec()  # scoring_end_block=20
    coordinator = NeverCompletingCoordinator()
    scorer.poll_open_benchmarks(conn, "comp1", spec, coordinator, current_block=25)
    scorer.poll_open_benchmarks(conn, "comp1", spec, coordinator, current_block=25)
